...
function getPageKeyFromPath(
  pathname: string
): 'home' | 'projects' | 'lastProject' | '404' {
  const path = pathname.replace(/\/+$/, '') || '/'
  if (path === '/' || path.endsWith('/index.html')) return 'home'
  if (path.endsWith('/projects') || path.endsWith('/projects.html'))
    return 'projects'
  if (path.endsWith('/last-project') || path.endsWith('/last-project.html'))
    return 'lastProject'
  ...
  switch (page) {
    case 'projects':
      return {
        ...
        description,
        keywords: [commonKeywords, t('seo.projects.keywords') as string]
          .filter(Boolean)
          .join(', '),
      }
    case 'lastProject':
      return {
        ...
        description,
        keywords: [commonKeywords, t('seo.lastProject.keywords') as string]
          .filter(Boolean)
          .join(', '),
      }
    case '404':
